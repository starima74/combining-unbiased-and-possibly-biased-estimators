...
out_bin_corr <- post_proc(X.1 = rbinom(N,1,prob=0.5),
                          ...
                          eps = rnorm(N))
saveRDS(out_bin_corr,"out_bin_corr2.rds")

out_bin_misp <- post_proc(X.1 = rbinom(N,1,prob=0.5),
                     X.2 =  rbinom(N,1,prob=0.5),
                     eps =  exp(rnorm(N))-1) 
saveRDS(out_bin_misp,"out_bin_misp2.rds")

out_cont_corr <- post_proc(X.1 = rnorm(N),
                      X.2 = rnorm(N),
                      eps = rnorm(N)) 
saveRDS(out_cont_corr,"out_cont_corr2.rds")

out_cont_misp <- post_proc(X.1 = rnorm(N),
                      X.2 = rnorm(N),
                      eps =  exp(rnorm(N))-1) 
saveRDS(out_cont_misp,"out_cont_misp2.rds")
...
TABLE2_cb <- data.frame(X1 = newdata$X.1,
                     X2 = newdata$X.2,
                     tau = c(rep(taus[1],4),rep(taus[2],4),rep(taus[3],4)),
                     theta_MVAR = round(truemse[,3],4),
                     thetamse_c2 = round(truemse[,5],4),
                     thetamse_c4 = round(truemse[,6],4),
                     thetamse_c8 = round(truemse[,7],4),
                     theta_MMSE  = round(truemse[,4],4))
...
TABLE2_mb <- data.frame(X1 = newdata$X.1,
                     X2 = newdata$X.2,
                     tau = c(rep(taus[1],4),rep(taus[2],4),rep(taus[3],4)),
                     theta_MVAR = round(truemse[,3],4),
                     thetamse_c2 = round(truemse[,5],4),
...
TABLE2_cc <- data.frame(X1 = newdata$X.1,
                     X2 = newdata$X.2,
                     tau = c(rep(taus[1],4),rep(taus[2],4),rep(taus[3],4)),
                     theta_MVAR = round(truemse[,3],4),
                     thetamse_c2 = round(truemse[,5],4),
...
TABLE2_mc <- data.frame(X1 = newdata$X.1,
                     X2 = newdata$X.2,
                     tau = c(rep(taus[1],4),rep(taus[2],4),rep(taus[3],4)),
                     theta_MVAR = round(truemse[,3],4),
                     thetamse_c2 = round(truemse[,5],4),
...
TABLE <- rbind(TABLE2_cb,TABLE2_cc,TABLE2_mb,TABLE2_mc)

TABLE2 <- TABLE[!(TABLE$X1==0 & TABLE$X2==1),]
